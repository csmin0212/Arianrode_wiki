...
const navItems = [
  { id: "overview", label: "티르밀리 개요", icon: "🧚" },
...
function OverviewSection() {
  return (
    <div>
      <Prose text={"에린의 대지를 모습을 바꾸며 살아가는 요정들의 거처는, 요정향이라 불리는 이세계라고 말해진다. 하지만 넓은 세계 속에는, 지상에 존재하는 요정들의 집락이 있다. 티르밀리는 그러한 요정 사회 중 하나다."} />
      <SecTitle title="개요" />
      ...
      <Prose text={"티르밀리는, 지상에 존재하는 요정의 거점 중에서도 특별한 의미를 가진다. 그것은 이 도시가, 요정왕 오베론에 의해 에린 안에 있는 모든 요정의 주거지와 연결하는 전이 장치로서 만들어졌기 때문이다. 오베론은 이 도시를 에린의 중계 기지로 정한 모양이다."} />
      <SecTitle title="현황" />
      ...
      <Prose text={"도시의 주변에는, 요정이라 불리는 마법의 나무가 있다. 이것들은 에린과 티르밀리를 연결하는 전이 장치이며, 도시를 황야에서 외부로부터 숨기는 마술 장치이기도 하다. 또한 도시의 바깥쪽에는, 밖에서 도시에 들어오려는 자가 있으면 적대적으로 그들을 추적하는, 도시의 수호를 위해 조직된 전사 집단이 기다리고 있다."} />
    </div>
  );
}
...
function CitySection() {
  const spots = [
    ...
    { name: "요정나무", jp: "妖精樹", desc: "거리 주변을 원환처럼 둘러싸듯이 서 있는 요정나무. 전체 12그루라고 한다. 이것들은 에린과 티르밀리를 연결하는 전이 장치이며, 도시를 황야에서 외부로부터 숨기는 마술 장치이기도 하다. 요정들은 이 나무를 사용하여 이 쪽에서 에린을 순찰하러 가기도 한다. 인간도 이 나무를 사용하는 것은 가능하다." },
  ];
  const facilities = [
    { name: "한가운데의 커다란 나무", jp: "まんなかの大きな木", desc: "거리 중앙에 있는 거목. 나무의 수령은 몇 백 년이나 됐는지 알 수 없다. 오래된 나무로 둘레의 나무 수는 수십 미터나 있어, 내부에는 커다란 우코의 공동이 있어, 티르밀리에 사는 요정들의 집 장소로 많은 경우는 도시의 주민들이 모여드는 연회 장소로서 이용되고 있다. 하지만 이 장소는, 요정왕 오베론이 로빈에게 이 도시의 자유를 위임한 대리공——「대보」 로빈이 사는 궁전이다. 평소에는 연회 장소의 주민들이 모이고 있지만, 로빈이 목소리를 낼 때에는, 대리공이 공사를 중단하고 명령을 전달하는 정치의 장소가 된다." },
  ];
  return (
    <div>
...
function OrgsSection() {
  const orgs = [
    { name: "전처녀기사단 발키리루루", type: "군사 조직", base: "디나시의 숲", leader: "「교훈하고 싶어하는」 아르비트", content: "티르밀리를 수호하는 방어 조직. 발키리로 구성된 전비 날아다니는 기사단이다. 도시 안의 요정이나, 도시를 위협하는 마수들에게 맞서기 위해 조직되어 있다. 주로 방위가 주요 임무이지만, 때에 따라 기사들을 외부에 파견하는 경우도 있다." },
    { name: "카하리셴", type: "정치 조직", base: "요정나무", leader: "「수호인」 에이라", content: "티르밀리를 에워싸는 요정나무를 관리하는 자들로, 다양한 요정나무를 중심으로 모여있는 거대한 집단. 디나시들을 중심으로, 다채다양하게 요정들로 구성되어 있다. 에린의 각지에 산재하는 요정나무를 관리하고, 나무를 이용하는 자들을 안내하고 있다." },
    { name: "샨노스 예단", type: "공연 단체", base: "스프라이트의 춤터", leader: "카린", content: "티르밀리에 존재하는 악단이나 시인·노래단을 포함하는 예능 단체. 도시 내에서 활동하고 있으며, 도시에 찾아온 인간이나 탐험자들을 대상으로 공연하는 경우가 있다. 악단으로서, 거리의 외부에서 행동하는 것도 없지는 않다." },
    { name: "그레믈린 도적단", type: "범죄 조직", base: "불명", leader: "잭래빗", content: "티르밀리에 사는 그레믈린이나 렙라콘으로 구성된 도적 부대. 정기적으로 마쥬라니카로부터 이상한 광석을 모으고 있다. 각 지방의 도중에 함정을 걸어 물건을 빼앗거나, 도중에 함정을 쳐서 인간들을 깊이 밀어넣는다." },
    { name: "펌프킨헤즈", type: "비밀 결사", base: "반시의 숲", leader: "불명", content: "티르밀리에 사는 잭·오·랜터른이나 고블린을 중심으로 하여 이루어진 비밀 결사. 숨어서 인간들을 놀라게 하려는 자들이 모이고 있다. 스스로의 계획이나 조직은 말하지 않는다. 리더는 좀처럼 사람들 앞에 나타나지 않는다." },
  ];
  return (
    <div>
      <SecTitle title="티르밀리의 조직" />
  ...
  const people = [
    {
      ...
      note: "티르밀리의 창설자이자 지배자인 요정왕 오베론에게 위임을 받아, 도시의 관리를 맡고 있는 요정인 「대보」의 로빈이라 불리는 것은, 오베론보다 한층 더 강하게 두드러지고, 머리의 회전이 빠르다고 알려져 있다. 수완은 좋지만 진지한 고민을 하기 싫어하고 있는 것처럼 보인다. 여유로운 태도로 담소를 즐기고 있다.",
    },
    {
      name: "「교훈하고 싶어하는」 아르비트",
      nameJp: "「教えたがり」アルヴィト",
      ...
      note: "티르밀리를 수호하는 기사단의 단장. 새하얀 날개를 가진 발키리들을 솔선하여 이끌고 있다. 기사단을 진행하기를 바라면서, 거친 성격에서 단원의 인원 집중이 모이고 있다. 기사단의 우두머리를 맡으려고 하는 수고로운 직무이기도 하나, 기억에 남기고, 박식한 교양을 갖고 있다고도 알려져 있다. 지식을 남에게 가르치고 싶어하는 경향이 있다.",
    },
    {
      name: "「수호인」 에이라",
      nameJp: "「守り人」エイラ",
    ...
    },
    {
      ...
      note: "티르밀리에 살아 버린 필보르의 여유 여행자. 성력 960년대에 도착했다고 하는 것이니까, 이미 50년 가까이 거주하고 있다. 요정들에 둘러싸인 생활이 몸에 들어와 버린 것이지만, 도시를 나오는 것도 싫지는 않다. 인간과 비슷한 성격으로, 새로 찾아온 인간을 안내하는 것이 취미이다.",
    },
  ];
  return (
    <div>
      <Prose text={"티르밀리에서 활동하는 주요 인물들."} />
    ...
    <div style={{ display: "flex", height: "100vh", fontFamily: "'Noto Sans KR', sans-serif", background: "#F7F4EE", color: "#2a2a2a" }}>
      ...
      {(!mob || showNav) && (
        <nav style={{ width: mob ? "100vw" : 220, flexShrink: 0, background: SIDEBAR_BG, display: "flex", flexDirection: "column", padding: "24px 0", overflowY: "auto", position: mob ? "fixed" : "relative", top: 0, left: 0, height: "100vh", zIndex: 999 }}>
          <a href="/" style={{ display: "block", padding: "0 20px 20px", textDecoration: "none" }}>
            ...
            <div style={{ fontSize: "15px", fontWeight: 700, color: "#fff", fontFamily: "'Noto Serif KR', serif" }}>티르밀리</div>
      ...
      <main ref={mainRef} style={{ flex: 1, overflowY: "auto", padding: mob ? "60px 16px 40px" : "40px 48px", maxWidth: 860 }}>
        <div style={{ marginBottom: 28 }}>
          ...
          <h1 style={{ fontFamily: "'Noto Serif KR', serif", fontSize: mob ? "22px" : "26px", fontWeight: 700, color: "#1a1a1a", margin: 0 }}>티르밀리</h1>
