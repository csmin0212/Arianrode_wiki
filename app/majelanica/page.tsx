...
function KingdomSection() {
  const lords = [
    { name: "사무드라 지방", lord: "니도라의 말예", race: "엘다난", note: "대륙 북서부 해안 지방. 니도라의 자손이 다스린다. 엘다난가 많이 모인다." },
    { name: "라사 지방", lord: "마우네야의 말예", race: "네바프", note: "대륙 서부 내륙의 라사 지방을 파라슈의 말예가 다스린다. 네바프가 많다." },
    { name: "아디 지방", lord: "나키라르의 말예", race: "필보르", note: "밀림 서남부에 넓은 아디 지방을 나키라르의 말예가 다스린다." },
    { name: "마루토와나 지방", lord: "사비야사르치의 말예", race: "버나", note: "대륙 중앙부에 넓은 마루토와나 지방을 사비야사르치의 말예가 다스린다." },
    { name: "네디에라 지방", lord: "드로나의 말예", race: "두앙", note: "대륙 동북부에 위치하는 네디에라 지방을 드로나의 말예가 관할한다." },
  ...
  return (
    <div>
      ...
      <Prose text={"6인의 번왕은 6인의 용사들의 자손. 6인의 번왕이 각각 다스리는 땅은, 동종족이 많이 집결하는 땅이기 때문에, 각 번왕의 지배 지역에는 동종족이 많이 살고 있다. 휴린의 번왕에게는 휴린이 많이 사는 영역에 번왕이 있다. 엘다난의 지배 지역에는 엘다난가 많이 사는 영역이 있다."} />
...
function KailashaSection() {
  const orgs = [
    ...
    { name: "샨타 목욕탕 상인 조합", type: "상인 조합", base: "비루-다키 구역", leader: "니바미히", content: "목욕탕이나 사우나, 온천이나 음식점, 치료 토산 전문점 등이 일체가 된 구역에 있는 조합. 비루-다키 항구의 상가가 거리가 되어 있는 목욕탕 거리의 상인 조합. 원래는 목욕탕을 경영하는 필보르의 일파로, 현재는 장로라 불리는 노인이 다스린다." },
    ...
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 8, marginBottom: 20 }}>
        {[
          ...
          { name: "구 시가지 (도리타라슈트람 구역)", desc: "구 시가지는 「성자의 거리」라고도 불린다. 이 구역에는 나레슈 신전을 비롯하여, 종교 시설이 몇 곳 건설되어 있다. 엘다난가 많이 거주하고 있다." },
        ].map((d, i) => (
          <div key={i} style={{ background: "#fff", border: "1px solid #E8E3DA", borderRadius: 10, padding: "14px 16px" }}>
            <div style={{ fontFamily: "'Noto Serif KR', serif", fontSize: "13px", fontWeight: 700, color: "#2a2a2a", marginBottom: 6 }}>{d.name}</div>
