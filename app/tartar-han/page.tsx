...
function HaruSection() {
  const places = [
    ...
    },
    {
      name: "창풍정 (蒼き風亭)",
      icon: "🍶",
      content: "동구 중앙 부근에 있는 식당 겸 주점. 가게는 필볼 여성이 경영하고 있으며, 후에는 서방과 동방 양쪽 요리에 정통하여 레퍼토리가 많다. 메뉴에는 동서의 다양한 음식이 줄지어 있으며, 가게 주인은 술을 좋아하지는 않지만 술의 종류는 항상 보충하고 있어 원하는 요리를 만드는 서비스도 있다. 맛과 양의 균형이 좋아 현지 평판도 높다.",
    },
  ];
...
function OrgsSection() {
  const orgs = [
    ...
    },
    {
      ...
      base: "자를루트 근교",
      leader: "아비카",
      content: "필볼의 소녀 아비카를 수령으로 하는 집단. 유목민이나 상인을 습격하는 산적 행위를 하며, 그 악명은 서방에도 알려져 있다. 사막의 길에서 활동하기가 어렵다.",
    },
    {
      name: "파-나도른 잔당",
      type: "비밀 결사",
  ...
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "'Noto Sans KR', sans-serif", background: "#F7F4EE", color: "#2a2a2a" }}>
      ...
      <nav style={{
        ...
        <div style={{ padding: "16px 20px", borderTop: "1px solid rgba(255,255,255,0.07)", fontSize: "11px", color: "#5A3A18" }}>
          <a href="/eastern-world" style={{ color: "#8A6A38", textDecoration: "none" }}>← 동방 세계</a>
        </div>
      </nav>
