...
const orgs: Org[] = [
  ...
  },
  {
    ...
    base: "현자의 광장 근처",
    leader: "오윈",
    content: "엘크레스트의 거리에 넓게 뻗은 상업 조합. 동쪽에서 찾아오는 상인들에 의해 운영되고 있다. 에를랑 왕국이나 뱅스타에서 찾아오는 행상인들의 중개 역할도 하며, 칼리지 내의 생활 협동 길드와도 상품을 조달하고 있다. 잡화나 무기 제품도 깊이 관련되고 있다.",
  },
];
...
function OverviewSection() {
  ...
  return (
    <div>
      ...
      <SecTitle title="에를랑·뱅스타·파리스 동맹 3국의 완충지대" />
      <Prose text={"에를랑 왕국, 뱅스타, 파리스 동맹의 3국이 강하게 맞붙는 완충 지대라는 지리적 관계에 있는 도시 국가이다. 에를랑 왕국과 신성 뱅스타 제국에 속하는 상대들이 중계 지점으로 이용하기 때문에 상업 도시로서도 주목받고 있다.\n\n가장 널리 알려진 이유는 도시 내에 전기제 대학인 엘크레스트 칼리지가 있다는 것이다. 로아셀 호수 탐사를 계기로 발전한 칼리지는, 대다수의 학자 수의 인재 양성 기관으로 알려져 있으며, 특히 정령마법 분야에서는 에린딜과도 두 번째를 다투는 것으로 알려져 있다. 거리에는 칼리지 입학을 지망하는 소년 소녀들이 많이 방문하며, 여름부터 가을에 걸쳐서의 수험·입학 시즌에는 거리 전체가 수험생들로 넘쳐난다." } />
      <SecTitle title="역사" />
      <Prose text={"에를랑·뱅스타·파리스 동맹 3국의 국경이 강하게 맞붙는 완충 지대라는 지리에 있는 도시 국가이지만, 평의회라는 조직을 중심으로 합의제 정치를 전개하고 있다. 어느 나라에도 속하지 않은 자유 공정한 조직임을 표방하고 있으며, 많은 연구자나 학생들, 혹은 그 자손만이 살면서 거리의 일상 상공업 제품은 마술 제품과 연금술 제품, 그것에 에를랑 왕국과 신성 뱅스타 제국에 속하는 상대들이 중계 지점으로 이용하기 때문에 상업 도시로서도 주목받고 있다.\n\n원래 작은 마을만이 있던 호수 주변에 100년 정도 전부터 서서히 사람이 모이기 시작하고, 도시가 형성됐다. 호수 기슭에 \"거리의 민\" 엘다가 행한다고 전해지는 고대 유적이 발견되어 연구자들이 많이 모이기 시작한 것이 계기였다." } />
    </div>
  );
}
