...
function OverviewSection() {
  ...
  return (
    <section>
      ...
      <Prose text="아발론은 이주신(二柱の女神, 두 기둥의 여신) 브리간티에와 아에마가 유계 안에 만든 소대지 위에 존재한다. 그 소대지의 외관은 인간이 아닌 신의 몸으로는 알 수 없으며, 안개가 낀 공간이 펼쳐진다고 한다." />
      <Prose text="아발론으로의 입구는 두 곳이다. 광장 정면에 있는 거대한 문이 정문이다. 또 하나는 문 반대편의 귀향항(帰郷港)으로, 여기에는 수척의 만능선이 정박해 있으며 안개 속을 부유하는 기묘한 모양새를 하고 있다. 물질계의 바다에서 안개 속으로 들어가면 유계의 바다로 나오며, 이 배들을 만능선(万能帆船)이라 부른다." />
    </section>
  )
}
...
function HistorySection() {
  return (
    <section>
      ...
      <Prose text="아발론은 이주신(二柱の女神, 두 기둥의 여신)에 의해 세워진 세계의 탑 중 하나다. 이 도시에는 신환자라 불리는 신들이 선택한 영웅들이 모여들었다." />
      ...
      <Prose text="아발론은 달의 여신 브리간티에와 강과 샘의 여신 아에마가 유계 위에 만들었다. 이 여신들로부터 다양한 지역에서 들어오는 다른 세계의 침략과 마족·요마(妖魔)가 에린딜로 나오는 것을 막기 위한 방책이었다." />
      <Prose text="여신들이 세계를 저지하는 「차원회랑」을 파괴해줄 인간에게 역할을 맡기기로 하고, 주신 아르켄라체에게 제안했다. 주신은 이를 받아들였고, 여신들은 유계에 일곱 개의 방새 도시를 건설했다. 아발론은 그 방새 도시 중에서 최초로 건설된 도시다." />
      ...
      <Prose text="현재 이 도시는 여신의 의사에 동조한 영웅들 — 신환자들이 거주하고 있다. 유계에서 물질계로 향하는 차원회랑에 대항하는 것이 환상 도시의 본래 역할이다." />
...
function CitySection() {
  const facilities = [
    {
      ...
      desc: "아발론의 거리 중심에 있는 신전. 달의 여신 브리간티에와 강과 샘의 여신 아에마를 모신다. 신전 앞 광장에는 이주신의 두 여신상이 세워져 있다. 신의 화신인 펠르샤 브리간티아르와 나인시스터즈 요정들이 지키는 경우가 많다.",
    },
    {
      name: "귀향항", nameJp: "帰郷港", icon: "⚓",
      desc: "아발론 유일의 항구. 아발론 북단의 작은 대지로, 「흰 안개의 배」를 비롯한 만능선이 정박해 있다. 항구 주변은 항상 유기(幽氣)로 가득 찬 안개로 뒤덮여 있으며 해면은 보이지 않는다. 항구에서 뛰어내린 자가 며칠 후 아무 기억도 없이 돌아왔다는 일화가 전해진다.",
    },
    {
  ...
  return (
    <section>
      ...
      <Prose text="각 방새 도시에는 수척씩 만능선이 배치되어 있다. 이 배는 유계를 자유롭게 항행할 수 있으며, 물질계와 오갈 수도 있다. 또한 공중 비행도 가능하고, 안개 정령의 바람을 받아 바람이 없어도 원하는 방향으로 나아갈 수 있다. 아발론의 「흰 안개의 배」가 가장 유명하다." />
...
function YukaiSection() {
  return (
    <section>
      <SecTitle title="유계란" />
      <Prose text="신들의 주거 신계(神界), 인간·동물들의 주거 물질계(物質界). 이 두 세계 사이에 존재하는 것이 유계다. 유계에는 시간이나 공간 같은 개념이 통용되지 않는다. 차원회랑이 시간·공간적으로 불안정한 성질을 가지는 이유도 이 때문이다." />
      <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 20 }}>
        {[
          ...
          { num: "②", title: "영혼의 정화",       desc: "물질계의 생물들의 영혼이 정화되는 장소. 물질계에서 선을 행한 자들은 유계로 불려가 정화되고, 다시 물질계에 새로운 생명으로 돌아간다. 이 정화의 모습이 천국과 지옥 이미지의 원천이 되었다고 한다." },
          { num: "③", title: "영웅의 승화",       desc: "영웅적 행위를 한 자가 신지신앙(신지신앙)을 받으면 신지에 더욱 가까워지고, 언젠가는 진정한 신이 되어 신계에 올라갈 수 있다." },
        ].map(r => (
      ...
      <Prose text="영적 세계인 유계에는 물질계와 같은 대지가 없었다. 그런데 언젠가부터 물질계와 같은 대지가 존재하게 되었다. 그 이유는 어느 시기에 물질계로 밀려든 요정들 때문이라고 한다. 요정들은 자신들이 관리하는 나무·집·자연 현상 등을 유계에 무의식적으로 만들어버렸다. 따라서 유계에도 일부이기는 하나 물질계와 같은 대지가 광범위하게 펼쳐지게 되었다." />
...
function GodcallersSection() {
  return (
    <section>
      ...
      <Prose text="다른 세계 침략자들이 에린에 침입하기 위해 만든 차원회랑의 대부분은 유계에 열린다. 유계에는 시간이나 공간 개념이 통용되지 않기 때문에 차원회랑은 불안정하다. 유계 내에서는 때때로 시간과 공간 개념이 혼란스러워지기도 한다." />
      <Prose text="아발론의 경우, 차원회랑 발생 알림은 신전원 원장인 펠르샤가 받는다. 펠르샤가 즉시 차원회랑으로 날아가 봉쇄·파괴한다. 펠르샤가 부재 중인 경우엔 그 부하 나인시스터즈가 대신한다." />

      <SecTitle title="미궁화된 차원회랑" />
      <Prose text="유계의 차원회랑은 때때로 변질되어 미궁이 된다. 변질된 차원회랑은 그야말로 미궁 — 던전화되어 침략자들이 신환자에게 이빨을 세우기 위한 수단으로 삼는 것이다." />
      <Prose text="차원회랑의 미궁 형태와 내용은 실로 다양하다. 차원회랑을 만든 내방자의 성격에 기인하는 경우가 많고, 더욱 많은 것은 침입한 신환자의 사고를 이용해 에린딜의 유적과 같은 모습을 현현시키는 것이다." />

      <SecTitle title="물질계에 열린 차원회랑" />
      <div style={{ background: "#1A0A08", border: "1px solid #8A3A1A", borderRadius: 8, padding: 14, marginBottom: 16 }}>
        ...
        <p style={{ color: "#D8CCA8", fontSize: 13, lineHeight: 1.7, margin: 0 }}>
          간혹 물질계에 차원회랑이 열리는 경우가 있다. 이것은 극히 위험한 상황이다.
          규모가 가장 작은 차원회랑이라도 복수의 고레벨 모험가가 아니면 대응할 수 없을 정도다.
          실제로 신들이 예언을 하사해 겨우 차원회랑을 막아낸 적도 과거에는 있었다.
        </p>
      </div>
      ...
      <Prose text="신환자의 수행 장소로서 대표적인 것은 방새 도시 안에 있는 시련장이다. 고대부터 살고 있는 위대한 영웅들이 모여 있어, 자신보다 강한 신환자에게 실력을 쌓아줄 수 있다. 유계라면 수행의 장소가 이가 없다." />

      <SecTitle title="시나리오에서의 활용" />
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {[
          { title: "레귤레이션", desc: "아발론을 무대로 플레이하는 경우, GM은 모든 에리어 데이터를 사용할 수 있도록 기본 캐릭터 레벨 20 이상을 추천한다. 전원 신환자 클라스로 아발론을 홈타운으로 삼아 플레이한다." },
          { title: "운명 클라스가 되기 위해 방문하다", desc: "운명 클라스로의 클라스 체인지를 위한 시련을 받기 위해 아발론을 방문하는 방법도 있다. 신지신앙을 받은 PC는 신환자 클라스 체인지가 가능하고, 그 내용은 차원회랑을 파괴·유계에 있는 고위 거인이나 용 같은 신들의 권속과 싸우는 것이다." },
  ...
  const avalonPeople = [
    {
      ...
      note: "「크라슈-노벨의 네 영웅」 중 한 명. 신화분신 아바타로서 용기억을 겨우 되찾아 오래 자리를 비운 아발론으로 귀환했다. 정령 마술을 특기로 하는 마법사로, 말수가 적고 사려 깊지만 뜬금없는 말을 하는 면이 있다.",
    },
    {
      ...
      quote: "데미지는 0이지☆",
      note: "일찍이 에린딜 대륙에서 마족과 요마와 싸운 세인(誓人) 중 한 명. 「아르디온 대륙의 대영웅」으로 불리는 사건 당시, 심홍의 아인·조우에 의해 구출되어 아발론으로 데려와졌다. 세인으로서의 맹세를 이루고, 현재는 신전원 수호단에 소속되어 시련장 지도를 맡고 있다.",
    },
    {
      ...
      <div style={{ display: "flex", minHeight: "100vh", fontFamily: "'Noto Sans KR', sans-serif", background: "#F7F4EE" }}>
        ...
        <main style={{ flex: 1, padding: "40px 48px", maxWidth: 860, overflowY: "auto" }}>
          <header style={{ marginBottom: 32 }}>
            <div style={{ fontSize: 12, color: "#8A7830", marginBottom: 4 }}>방새 도시 · 유계</div>
