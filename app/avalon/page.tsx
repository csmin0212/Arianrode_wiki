...
function HistorySection() {
  return (
    <section>
      ...
      <Prose text="에린디일의 여러 지역에는 요정·아바르·안개 지킴이 같은 다양한 존재들이 있으며, 행방을 알 수 없는 곳도 있다. 적어도 그 일부는 「영원의 거리」 아발론의 것이라는 공통된 이야기가 전해진다." />
      <SecTitle title="창설의 경위" />
      <Prose text="아발론은 月の女神 브리간티에와 河と泉の女神 아에마가 유계(幽界) 위에 만들었다. 이 여신들로부터 다양한 지역에서 들어오는 다른 세계의 침략과 마족·요마(妖魔)가 에린디일로 나오는 것을 막기 위한 방책이었다." />
...
function CitySection() {
  const facilities = [
    ...
    },
    {
      ...
      desc: "아발론 유일의 주점. 항상 활기가 넘쳐 밤낮 없이 대단한 신환자들로 북적인다. 에린디일·아르디온 등지의 음식이 갖춰져 있다. 점주 가르파가 조달한다고 여겨지지만, 주인이 가게를 비운 적이 없다. 에린디일과 아르디온의 각지에서 비슷한 사람이 주인을 맡는 가게가 있다고 한다.",
    },
    {
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {[
          ...
          },
          {
            name: "リエナンテ",
            title: "「방황의 거리」 리에나테",
            desc: "에린디일 서쪽 방면에서 쫓겨다니고 있다고 소문난 거리. 그래서 「방황의 거리」 두 가지 이름으로 불린다. 모험가나 상인 등 비교적 많은 사람들에게 목격되며, 안개와 함께 나타나고 안개가 걷히면 사라진다고 한다. 리에나테에는 신들이 남긴 신구가 있다고 전해지며, 그것을 찾아 모험가들이 이 거리를 탐색하고 있다.",
          },
          {
            name: "アヴァル",
            title: "아발 (アヴァル)",
...
function GodcallersSection() {
  return (
    <section>
      ...
      <Prose text="次元回廊의 미궁 형태와 내용은 실로 다양하다. 次元回廊을 만든 來訪者의 성격에 기인하는 경우가 많고, 더욱 많은 것은 침입한 신환자의 사고를 이용해 에린디일의 유적과 같은 모습을 현현시키는 것이다." />
...
function SistersSection() {
  const cities = [
    { num: "壱", name: "아발론", nameJp: "アヴァロン",      status: "건재",   color: ACCENT,   area: "에린디일 서방 + 아르디온 서방 + 중앙 담당" },
    { num: "弐", name: "호우라이", nameJp: "ホウライ",      status: "건재",   color: "#4A9A4A", area: "에린디일 동방 담당. 신환자는 에린디일 동방의 세리아 대제국 출신이 많다." },
    ...
    { num: "肆", name: "자나두", nameJp: "ザナドゥ",         status: "함락",   color: "#8A3A1A", area: "에린디일 대륙 중앙 담당. 기동력·공격력이 가장 높은 도시였다. 사상 최초로 침략자에게 함락된 방새 도시 — 성력 753년." },
    ...
    { num: "柒", name: "티르·나·노그", nameJp: "ティル·ナ·ノグ", status: "함락", color: "#8A3A1A", area: "에린디일 대륙 북방 담당. 요정과 친화성이 높은 도시. 성력 799년 함락." },
  ]
  return (
    <section>
      ...
      <Prose text="에린디일 서방과 아르디온 동서방을 아발론이, 에린디일 동방과 남방 대륙을 호우라이가, 샴발라는 잔존 2개 도시의 백업을 담당하는 체제가 되었다." />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 8 }}>
        {[
          ...
          },
          {
            ...
            icon: "❓",
            desc: "에린디일 서방 각지를 방황하는 전설의 거리. 안개와 함께 나타나고 안개가 걷히면 사라진다. 방새 도시가 물질계로 이동할 때의 현상과 유사하며, 아발론은 이 거리의 정체가 폭산한 엘·도라도의 잔해일 가능성을 검토하고 있다. 내부에 다수의 신구가 보관되어 있다는 전설도 있다.",
          },
        ].map(f => (
...
function PeopleSection() {
  const avalonPeople = [
    ...
    },
    {
      ...
      quote: "데미지는 0이지☆",
      note: "일찍이 에린디일 대륙에서 마족과 요마와 싸운 세인(誓人) 중 한 명. 「아르디온 대륙의 대영웅」으로 불리는 사건 당시, 深紅의 아인·조우에 의해 구출되어 아발론으로 데려와졌다. 세인으로서의 맹세를 이루고, 현재는 신전원 수호단에 소속되어 시련장 지도를 맡고 있다.",
    },
    {
      ...
      note: "웨이랜드 단금 공방의 대표. 신구 이외라면 어떤 마법 아이템도 만들어낼 수 있다고 호언장담한다. 에린디일 서방에서 모르는 자가 없다는 전설의 단금 술사 티안의 직제자로, 그 기술을 이을 수 있다고 한다.",
    },
    {
      ...
      note: "「성배의 인도」 주점의 점주. 수수께끼 많은 인물로, 그가 모르는 것은 없다고 여겨질 만큼의 정보통이다. 아르디온 대륙의 헥스포드 거리를 비롯해 에린디일과 아르디온의 각지에서 그와 비슷한 사람을 만난 모험가도 적지 않다.",
    },
    {
  ...
  const otherCityPeople = [
    ...
    {
      name: "베네트", nameJp: "ベネット",
      role: "삼하 여신 (에린의 안전을 지키는 여신)",
      race: "바르나 (늑대족)", gender: "여", age: "?",
      hair: "녹색", eyes: "녹색", skin: "백색",
      accent: "#2A7A8A",
      quote: "삼하 여신이란 바로 나 말이야 그랬어요!!",
      note: "원래 별것 없는 부하였던 베네트였지만, 수많은 모험과 업적을 이루어 여신 아리안로드의 화신 펠르샤의 천거로 에린의 안전을 지키는 여신이 되었다. 그러나 지금도 그 「삼하」 티가 변하지 않아, 지금도 「나」「~했어요」라는 말투가 빠지지 않는 삼하 여신이다.",
    },
  ]
  const invaders = [
    {
